...
class BearViewController: UIViewController {
    ...
    func addMenuViewController() {
        if menuNavController == nil {
            ///FIXME: 是否是指向同一个实例？？
            guard let menuNavController = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "MenuNavController") as? MenuNavController
                else {return}
            // 插入当前视图并置顶
    ...
    func animateMainView(shouldExpand: Bool) {
        ...
        if shouldExpand {
            ...
            mainNavController?.view.addGestureRecognizer(tapGestureRecognizer!)
            // 设置主页面不可交互
            (mainNavController?.children[0] as? CategoryTableViewController)?.tableView.isScrollEnabled = false
        }
        // 如果用于隐藏
        else {
            // 动画
